...
\begin{adjustwidth}{113pt}{43pt}
    ...
    \sortitem{LSB - Least Significant Bits}
	\end{sortedlist}
\end{adjustwidth}
...
An example is \codeline{
\texttt{[TCP:flags:A]-duplicate(tamper\{TCP:flags:replace:R\}(tamper\{TCP\\:chksum:corrupt\},),)-| \textbackslash/}
}
...
\subsubsection{Method Implementation}
In the practical implementation of the Geneva algorithm, I specifically executed the Geneva engine using the strategy \codeline{[TCP:flags:PA]-fragment\{tcp:8:True\}(,fragment\{tcp:4:True\})-|}. This strategy was selected for its potential to intricately manipulate TCP traffic and effectively evade DPI systems used for Internet censorship.
...
\hline
\textbf{Method} & \textbf{Detection Resistance} & \textbf{Client-Server Access} & \textbf{Latency Impact} & \textbf{Throughput} \\ \hline
Encryption Tunneling & Pass & Server required & +6\% & -10\% \\ \hline
Geneva & Pass & Client only & +14\% & -11\% \\ \hline
Steganography & Pass & Server required & +560\% & -88\% \\ \hline
Port Switching & Fail & Server required & No Impact & No Impact \\ \hline
\end{tabular}
}
\end{table}
